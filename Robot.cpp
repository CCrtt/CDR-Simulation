...
void robot::update(sf::Time time)
{
    if (m_delay==0)
    {
        ...
        actualise_position(m_rightSpeed*22/17, m_leftSpeed*22/17);
...
void robot::startGen(double x, double y) // appelée dans le main, setup tout avant de lancer la recherche de trajectoires
{
    ...
    m_posTest.Setall(m_pos.getX(), m_pos.getY(), m_pos.getangle());
    actualisePositionTest(m_rightSpeed*TEMPS_GEN/17, m_leftSpeed*TEMPS_GEN/17, -1);
    m_posDep.Setall(m_posTest.getX(), m_posTest.getY(), m_posTest.getangle());

    //remise a 0 des scores des sol
    m_sol[0][NB_TOURS_SIMULES][0]=-100000;
    m_sol[1][NB_TOURS_SIMULES][0]=-100000;
    m_sol[2][NB_TOURS_SIMULES][0]=-100000;
    m_sol[3][NB_TOURS_SIMULES][0]=-100000;
    m_sol[4][NB_TOURS_SIMULES][0]=-100000;
    ...
    //on peut ajouter d'autres mecanismes de generation de sol initiales si besoin
    ...
    for (int j=0; j<NB_SOL; j++)
    {
        ...
        {
            actualisePositionTest(m_sol[j][i][0]*200/17, m_sol[j][i][1]*200/17, i);
...
{
    //echange les positions de sol1 et sol2. Optimisable en le faisant avec des pointeurs, mais cette fonction est finalement assez pau utilisee
    if (sol1 != sol2)
    {
...
void robot::genSol()
{
    ...
    double a = (double) rand()/RAND_MAX*2.0;
    int ind1 = floor(a*a+0.07);
    a = (double) rand()/RAND_MAX*2.0;
    int ind2 = floor(a*a+0.07);
    ...
    for (int i=0; i<NB_TOURS_SIMULES; i++)
    {
        actualisePositionTest(m_sol[NB_SOL][i][0]*200/17, m_sol[NB_SOL][i][1]*200/17, i);
    ...
    //on place la sol trouvee parmi les sol existantes en la comparant à chacune
    int classement = 5;

    if (m_sol[NB_SOL][NB_TOURS_SIMULES][0]>m_sol[4][NB_TOURS_SIMULES][0] && m_collision == false)
    {
        if (m_sol[NB_SOL][NB_TOURS_SIMULES][0]>m_sol[1][NB_TOURS_SIMULES][0])
        {
            if (m_sol[NB_SOL][NB_TOURS_SIMULES][0]>m_sol[0][NB_TOURS_SIMULES][0])
                classement=0;
            else
                classement=1;
        }
        else if (m_sol[NB_SOL][NB_TOURS_SIMULES][0]>m_sol[3][NB_TOURS_SIMULES][0])
        {
            if (m_sol[NB_SOL][NB_TOURS_SIMULES][0]>m_sol[2][NB_TOURS_SIMULES][0])
                classement=2;
            else
                classement=3;
        }
...
void robot::testCollisionTest()
{
    ...
    if (dist(m_posTest.getX() - WINDOW_WIDTH/2, m_posTest.getY() - (WINDOW_HEIGHT - 300)) < sqrt(m_width*m_width + m_length*m_length)/2) //collision avec le bout de la balance
    {
        m_malus += 200;
        m_collision = true;
    }
...
{
    if (dist(m_target.getX()-m_pos.getX(), m_target.getY()-m_pos.getY())<13 && val_abs(properAngleRad(m_target.getangle()-m_pos.getangle()))<0.05 && val_abs(m_rightSpeed) < 0.2 && val_abs(m_leftSpeed) < 0.2)
        return true;
    return false;
}
